...
unsigned char Lpwm_val = 180; // Speed ranges from 0-255
...
void increaseSpeed() // If speed is too high then speed will not be changed
    {
      if (Lpwm_val >= 220) {
        return;
      }
...
void decreaseSpeed() // If speed is too low then speed will not be changed
    {
...
void loop() 
    ...
    if(Serial.available()) //to judge whether the serial port receives the data.
        ...
        break;
        case 'H':increaseSpeed();   // Speedup
        Serial.println("SPEEDUP");
        break;
        case 'G':decreaseSpeed();   // Slowdown 
